...
function dibujarGuias(ctx: Ctx, guias: Guia[], ancho: number, alto: number, escala: number) {
  guias.forEach((g) => {
    if (!g.visible) return;
    ctx.save();
    ctx.setLineDash([10 * escala, 5 * escala]);
    ctx.strokeStyle = g.id === 'guia-media' ? 'rgba(59,130,246,0.5)' : 'rgba(167,139,250,0.5)';
...
function dibujarZonaSonrisa(ctx: Ctx, faceData: FaceData, escalaX: number, escalaY: number) {
...
export async function renderComposito(config: ConfigRender): Promise<ResultadoRender> {
  const { foto, dientes, guias, faceData, ancho, alto, formato, calidad, incluirGuias } = config;
  ...
  // ── 1. Foto base ──────────────────────────────────────────────────────
  const imgScale = Math.min(ancho / foto.naturalWidth, alto / foto.naturalHeight);
  const imgX     = (ancho - foto.naturalWidth  * imgScale) / 2;
  const imgY     = (alto  - foto.naturalHeight * imgScale) / 2;
  ctx.drawImage(foto, imgX, imgY, foto.naturalWidth * imgScale, foto.naturalHeight * imgScale);
  ...
  ctx.textAlign = 'right';
  ctx.fillText('Smile Design PRO · Digital Mockup', ancho - 16 * escala, alto - 12 * escala);
  ctx.restore();
