...
import type { FaceData, Landmarks } from './tipos-faciales';
...
export function analizarVisagismo(faceData: FaceData): ResultadoVisagismo {
  ...
  const indiceMandibular = anchoPomulos > 0 ? anchoMandibula / anchoPomulos : 0.8;
  ...
  const { preset, morfologia, razonamiento, confianza } = mapearPreset(
    forma, indiceFacial, indiceMandibular, anguloFacial
  );
...
function mapearPreset(
  ...
  indiceMandibular: number,
  anguloFacial: number
): { preset: PresetVisagismo; morfologia: 'oval' | 'cuadrado'; razonamiento: string; confianza: number } {
  ...
  switch (forma) {
    case 'oval':
      return {
        preset:      'premium',
        morfologia:  'oval',
        razonamiento: 'Rostro oval: morfología ideal para dientes amplios y redondeados. Preset Premium maximiza la armonía.',
    ...
    case 'cuadrada':
      return {
        preset:      'natural',
        morfologia:  'cuadrado',
        razonamiento: 'Rostro cuadrado: dientes con bordes incisales más planos y forma cuadrada equilibran la mandíbula prominente.',
    ...
    case 'triangular':
      return {
        preset:      'suave',
        morfologia:  'oval',
        razonamiento: 'Rostro triangular: dientes más estrechos y suaves en cervical para no acentuar el mentón.',
    ...
    case 'redonda':
      return {
        preset:      'premium',
        morfologia:  'oval',
        razonamiento: 'Rostro redondo: dientes más alargados y estilizados (preset Premium) para dar verticalidad al perfil.',
        confianza:   Math.max(0.1, 0.80 - penalizacionAngulo),
      };
    ...
    default:
      return {
        preset:      'natural',
        morfologia:  'oval',
        razonamiento: 'Forma facial no determinada con certeza. Se sugiere preset Natural como punto de partida neutro.',
...
export const DESCRIPCION_FORMA: Record<FormaFacial, string> = {
  ...
  cuadrada:      'Cuadrada — mandíbula prominente, favorece dientes con bordes planos',
  triangular:    'Triangular — frente ancha y mentón estrecho, favorece dientes suaves',
  redonda:       'Redonda — proporciones similares en ancho y alto, favorece dientes alargados',
