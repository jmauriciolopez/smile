...
export function useFaceEngine(): UseFaceEngineReturn {
  ...
  const detectar = useCallback(async (imagenEl: HTMLImageElement) => {
    setDetectando(true);
    setError(null);
    ...
    try {
      ...
      if (!resultado?.faceLandmarks?.length) {
        setError('No se detectó ningún rostro. Intenta con una foto más clara.');
        return;
      }
      ...
      const bocaCentro: Vec2 = {
        ...
        y: (toVec2(MP_IDX.boca_centro_sup).y + toVec2(MP_IDX.boca_centro_inf).y) / 2,
      };
      ...
      const distInterocular = Math.hypot(ojDer.x - ojIzq.x, ojDer.y - ojIzq.y);
...
function alinearDientesAlRostro(face: FaceData, store: ReturnType<typeof useEditorStore.getState>) {
  const { lips, transform } = face;
  const { dientes } = store;
  if (!dientes.length) return;

  // Centro de los dientes = centro de la boca
  const centroX = lips.boundingBox.x + lips.boundingBox.width / 2;
  const baseY   = lips.boundingBox.y;
  const anchoTotal = dientes.length > 0
    ? dientes[dientes.length - 1].transform.x - dientes[0].transform.x +
      dientes[dientes.length - 1].transform.scaleX * 100
    : lips.boundingBox.width;

  dientes.forEach((d, idx) => {
    const fraccion = (idx / (dientes.length - 1)) - 0.5; // -0.5..+0.5
    useEditorStore.setState(s => ({
      dientes: s.dientes.map(x =>
        x.id === d.id
          ? {
              ...x,
              transform: {
                ...x.transform,
                x:        centroX + fraccion * anchoTotal,
                y:        baseY - 10,
                rotation: transform.rotation, // alinear al eje facial
              },
            }
          : x
      ),
