...
export async function analizarColorMatch(
  ...
  const muestraIzq = muestrearEsclera(ctx, ojIzq.x * escalaX, ojIzq.y * escalaY, 12 * escalaX);
  const muestraDer = muestrearEsclera(ctx, ojDer.x * escalaX, ojDer.y * escalaY, 12 * escalaX);
...
function muestrearEsclera(
  ...
  const r = Math.round(muestras.reduce((s, m) => s + m[0], 0) / muestras.length);
  const g = Math.round(muestras.reduce((s, m) => s + m[1], 0) / muestras.length);
  const b = Math.round(muestras.reduce((s, m) => s + m[2], 0) / muestras.length);
...
function calcularSugerencia(r: number, g: number, b: number): ResultadoColorMatch {
