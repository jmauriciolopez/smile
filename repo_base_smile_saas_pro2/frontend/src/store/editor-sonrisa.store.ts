...
import { BIBLIOTECA_DENTAL, PIEZAS_FRONTALES, generarPosicionesIniciales } from '../motor/biblioteca-dientes';
...
function crearSnapshot(dientes: Diente[], guias: Guia[]): BlueprintVersion {
  const dientesSnap = dientes.map(({ svgPath: _, ...rest }) => rest);
  return {
...
export const useEditorStore = create<EditorStore>((set, get) => ({
  ...
  guias: [
    { id: 'guia-media',      tipo: 'vertical',   posicion: { x: 430, y: 0   }, visible: true  },
    { id: 'guia-oclusal',    tipo: 'horizontal', posicion: { x: 0,   y: 290 }, visible: true  },
    { id: 'guia-bipupilar',  tipo: 'horizontal', posicion: { x: 0,   y: 160 }, visible: false },
  ],
  ...
  toggleVisibilidad: (id) => {
    get().guardarSnapshot();
    set((s) => ({
      dientes: s.dientes.map((d) => (d.id === id ? { ...d, visible: !d.visible } : d)),
    }));
  },
  ...
  resetDientes: () => {
    ...
    const posiciones = generarPosicionesIniciales(canvasSize.width, canvasSize.height);
    ...
    set({ dientes: iniciales, seleccionadoId: null, history: [], historyIndex: -1 });
  },
  ...
  guardarSnapshot: () => {
    ...
    const nuevaHistory = [...history.slice(0, historyIndex + 1), snapshot].slice(-50);
    ...
    generarHash({ dientes: snapshot.dientes, guias: snapshot.guias }).then((hash) => {
      set((s) => ({
  ...
  undo: () => {
    const { history, historyIndex, dientes } = get();
    ...
    set({ dientes: dientesRestaurados, guias: prev.guias, historyIndex: historyIndex - 1 });
  },
  ...
  redo: () => {
    ...
    set({ dientes: dientesRestaurados, guias: next.guias, historyIndex: historyIndex + 1 });
  },
  ...
  importarDiseno: (json) => {
    ...
    set({ dientes, guias: json.guias ?? get().guias, history: [], historyIndex: -1 });
  },

  exportarDiseno: () => {
    const { dientes, guias } = get();
    const dientesExport = dientes.map(({ svgPath: _, ...rest }) => rest);
  ...
  exportarFichaTecnica: (): FichaTecnica => {
    ...
    const piezas: MedidaPieza[] = dientes.map((d) => {
      ...
      const anchoMm = calibracion ? +(anchoPx * calibracion.mmPorPx).toFixed(2) : undefined;
      const altoMm  = calibracion ? +(altoPx  * calibracion.mmPorPx).toFixed(2) : undefined;
      return {
        pieza:   d.pieza,
